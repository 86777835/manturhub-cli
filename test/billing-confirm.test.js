...
test("formats, confirms and reports billing", async () => {
  ...
  input.end("y\n");
  assert.equal(await confirmCharge({ estimated_dumplings: 85, formula: "17×5", balance: 100 }, { input, output }), true);

  let rendered = "";
  printBillingResult({ _billing: { estimated_dumplings: 85, charged_dumplings: 83,
    refunded_dumplings: 2, final: true } }, { write: (value) => { rendered += value; } });
